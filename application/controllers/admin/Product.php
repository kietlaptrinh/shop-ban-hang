        ...
        public function update($id)
{
    ...
    if ($product === null) {
        $this->session->set_flashdata('error', 'Sản phẩm không tồn tại hoặc đã bị xóa vào thùng rác!');
        redirect('admin/product', 'refresh');
    }

    // Gọi product_detail với alias
    $this->data['row'] = $this->Mproduct->product_detail($product['alias']);
    ...
    if ($this->form_validation->run() == TRUE) {
        $mydata = array(
            ...
            'producer' => $_POST['producer'],
            'name' => $_POST['name'],
            'alias' => $this->alias->str_alias($_POST['name']),
            'detail' => $_POST['detail'],
            'sortDesc' => $_POST['sortDesc'],
            'sale' => $_POST['sale_of'],
            'price' => $_POST['price_root'],
       ...
       public function import($id)
{
    ...
    // Gọi product_detail với alias
    $row = $this->Mproduct->product_detail($product['alias']);
    ...
    $this->load->library('session');
    $this->load->library('alias');
    $this->form_validation->set_rules('number', 'Số lượng', 'required|numeric|greater_than[0]');
