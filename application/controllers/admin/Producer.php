...
class Producer extends CI_Controller {
	...
	public function restore($id)
	{
		...
		$this->session->set_flashdata('producer_success', 'Khôi phục nhà cung cấp thành công');
		redirect('admin/producer/recyclebin','refresh');
	}
	public function delete($id)
{
    // Kiểm tra xem nhà cung cấp có sản phẩm liên quan không (kể cả trong thùng rác)
    $this->db->where('producer', $id);
    $query = $this->db->get('db_product');
    if ($query->num_rows() > 0) {
        $this->session->set_flashdata('error', 'Không thể xóa nhà cung cấp vì còn sản phẩm liên quan (kể cả trong thùng rác)! Vui lòng xóa vĩnh viễn các sản phẩm liên quan trước.');
        redirect('admin/producer/recyclebin', 'refresh');
    } else {
        $this->Mproducer->producer_delete($id);
        $this->session->set_flashdata('producer_success', 'Xóa nhà cung cấp thành công');
        redirect('admin/producer/recyclebin', 'refresh');
    }
}
