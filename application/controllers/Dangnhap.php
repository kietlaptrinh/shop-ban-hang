...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='Website - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->to($email);
            $this->email->subject('Hệ thống Website - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng Website, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
        ...
        $this->data['title']='Website - Đăng ký tài khoản';   
...
//laylaipass
    ...
    if($this->form_validation->run() == TRUE){
        ...
        $config['smtp_port']    = '465';
        $config['smtp_timeout'] = '7';
        $config['smtp_user']    = 'duongtuankiet.dev@gmail.com';
        $config['smtp_pass']    = 'zldm kazo dmqu yumn';
        $config['charset']      = 'utf-8';
        $config['newline']      = "\r\n";
        $config['wordwrap']     = TRUE;
        $config['mailtype']     = 'html';
        ...
        $this->email->from($config['smtp_user'], 'SORATECH'); 
        $this->email->to($list['email']);
        $this->email->subject('Website - Yêu cầu lấy lại mật khẩu');
        
        // Tạo link reset mật khẩu
        $reset_link = base_url().'dangnhap/reset_password_new/'.$list['id'];
        $message = 'Chào bạn,<br><br>';
        $message .= 'Chúng tôi nhận được yêu cầu lấy lại mật khẩu cho tài khoản của bạn.<br>';
        $message .= 'Vui lòng nhấp vào đường dẫn dưới đây để đặt lại mật khẩu mới:<br>';
        $message .= '<a href="'.$reset_link.'" style="display:inline-block; background-color:#007bff; color:white; padding:10px 15px; text-decoration:none; border-radius:5px; margin-top:10px;">Lấy lại mật khẩu</a><br><br>';
        $message .= 'Nếu bạn không thực hiện yêu cầu này, vui lòng bỏ qua email này.<br><br>';
        $message .= 'Trân trọng,<br>Đội ngũ Website.';

        $this->email->message($message);

        
        if ($this->email->send()) {
            $this->data['success'] = 'Yêu cầu đã được gửi. Vui lòng kiểm tra email để lấy lại mật khẩu!';
        } else {
            
            $this->data['error'] = 'Không thể gửi email. Vui lòng thử lại sau.';
            
        }

    }

    $this->data['title'] = 'Website - Quên mật khẩu';
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='Website - Cập nhật mật khẩu mới';
